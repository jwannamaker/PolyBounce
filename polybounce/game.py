...
from polybounce.polygon import Polygon
from polybounce.ball import Ball

class PolyBounce:
    '''
    End Product:
        The Rings will: 
            Continually generate with random N = [3 - 20]
    ...
    def __init__(self):
        ...
        self.inner_ring = Polygon(radius=250, N=6)
        self.mid_ring = Polygon(radius=300, N=6)
        self.outer_ring = Polygon(radius=350, N=6)
        self.ring_group = pygame.sprite.Group(self.inner_ring, self.mid_ring, self.outer_ring)
        
        self.player_ball = Ball(radius=25)
